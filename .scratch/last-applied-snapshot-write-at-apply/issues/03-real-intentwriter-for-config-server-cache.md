...
- [x] Unit test: delete-apply removes the last-applied entry immediately; the next `LoadAllButRunningIntents` does not rehydrate it (the ghost-intent regression signature) — `TestConfigServerCache_InstanceIntentDelete_RemovesFromSeam` (`pkg/cache/configserver_test.go`)
...
- [x] Unit test: rollback (`TransactionSet` re-run on old intents) restores a deleted entry — `TestTransactionRollback_RestoresDeletedIntent` (`pkg/datastore/transaction_rpc_test.go`)
