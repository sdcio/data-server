...
**Blocked by:** 01 (device-profile config field, `DeviceProfileSonic` constant, JSON_IETF-only validation), 03 (sonic encoder package)

**Status:** done

- [x] `materialize.BuildPlan` gains a branch: `sbi.Type == config.SBITypeGnmi && sbi.DeviceProfile == config.DeviceProfileSonic` calls the new encoder's `Encode(...)`, mirroring exactly how the existing `DeviceProfileCiscoIOSXR` branch calls `permodule.Encode(...)`.
...
- Landed on branch `sonic-device-profile`, commit `90c8796`: added `sbi.DeviceProfile == config.DeviceProfileSonic` dispatch branch in `materialize.BuildPlan` with two routing tests in `materialize_test.go`. Subsequent refactor commits (`577ee93`–`d05c944`) removed the `IsSonic()`/`IsCiscoIOSXR()` middle-man predicates, exported `SBITypeGnmi`/`SBITypeNetconf`/`SBITypeNoop` constants, and extracted the `gnmiPlan` error-wrap helper.
