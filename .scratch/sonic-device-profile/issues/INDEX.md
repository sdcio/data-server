...
| [02](02-sdc-protos-device-profile-sonic-enum.md) | sdc-protos: add `DEVICE_PROFILE_SONIC` to the `DeviceProfile` enum | None | ready-for-agent |
...
2. Among those, pick the lowest-numbered one — that's the frontier ticket. Right now 01, 02, and 03 are all unblocked and can be worked in any order (or in parallel).
