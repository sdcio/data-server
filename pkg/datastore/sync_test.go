...
// buildFixtureIntent renders a ygot device fixture into a tree_persist.Intent
// via the real import/export path, so tests exercise the same code a real
// intent would go through rather than hand-built proto literals.
func buildFixtureIntent(t *testing.T, scb schemaClient.SchemaClientBound, name string, priority int32, device *sdcio_schema.Device) *tree_persist.Intent {
	...
	_, err = root.ImportConfig(ctx, &sdcpb.Path{}, jsonImporter.NewJsonTreeImporter(contentAny, name, priority, false), types.NewUpdateInsertFlags(), vpf)
	if err != nil {
