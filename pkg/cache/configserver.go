...
func (c *ConfigServerCache) InstanceCreate(ctx context.Context, cacheInstanceName string) error {
	...
	// Seed with an empty Intent rather than nil: InstanceRunningGet has
	// nothing special to do for "not yet modified" beyond returning that
	// empty value, same as a freshly created LocalCache instance's running
	// store starts out with no data written to it yet.
