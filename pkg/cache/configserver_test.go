...
// TestConfigServerCache_InstanceRunningGet_NotYetModified verifies
// InstanceRunningGet on a freshly created instance returns an empty
// (zero-value) adapter rather than an error — InstanceCreate seeds
// "running" with an empty Intent, same as a freshly created LocalCache
// instance's running store starts out with no data written to it yet.
func TestConfigServerCache_InstanceRunningGet_NotYetModified(t *testing.T) {
	...
	if gotName := got.GetName(); gotName != "" {
		t.Errorf("InstanceRunningGet().GetName() = %q, want empty", gotName)
	}
}
