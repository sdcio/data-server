...
func (s *sharedEntryAttributes) DeepCopy(tc api.TreeContext, parent api.Entry) (api.Entry, error) {
	...
	// copy leafvariants
	result.leafVariants = s.leafVariants.DeepCopy(tc.OperationState(), result)
...
func (s *sharedEntryAttributes) tryLoadingDefault(ctx context.Context, path *sdcpb.Path) (api.Entry, error) {
	schema, err := s.treeContext.TreeConfig().SchemaClient().GetSchemaSdcpbPath(ctx, path)
	if err != nil {
...
func (s *sharedEntryAttributes) populateSchema(ctx context.Context) error {
	...
	if getSchema {
		...
		schemaResp, err := s.treeContext.TreeConfig().SchemaClient().GetSchemaSdcpbPath(ctx, path)
		if err != nil {
			return err
		}
		s.schemaMutex.Lock()
