	...
	tests := []struct {
	...
	}{
		{
			...
			sharedEntryAttributes: func() *sharedEntryAttributes {
				...
				result.leafVariants = api.NewLeafVariants(tc.GetOperationState(), result)
		...
		},
		{
			...
			sharedEntryAttributes: func() *sharedEntryAttributes {
				...
				interf.leafVariants = api.NewLeafVariants(tc.GetOperationState(), interf)
		...
		},
		{
			...
			sharedEntryAttributes: func() *sharedEntryAttributes {
				...
				system.leafVariants = api.NewLeafVariants(tc.GetOperationState(), system)
	...
	}{
		{
			...
			fields: fields{
				sharedEntryAttributes: func(t *testing.T) *sharedEntryAttributes {
					...
					schema, err := tc.GetTreeConfig().SchemaClient().GetSchemaSdcpbPath(ctx, nil)
					if err != nil {
						t.Fatal(err)
					}
