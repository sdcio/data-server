...
func TestLeafVariants_GetHighesPrio(t *testing.T) {
	...
	t.Run("Delete Non New",
		func(t *testing.T) {
			lv := api.NewLeafVariants(api.NewTreeOperationState(), nil)
...
func Test_RevertNonRevertive(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = root.ImportConfig(ctx, nil, runningConfig, types.NewUpdateInsertFlags(), tc.TreeConfig().PoolFactory())
			if err != nil {
				t.Fatalf("failed to import running config: %v", err)
			}

			for _, existing := range existingConfig {
				_, err = root.ImportConfig(ctx, nil, existing, types.NewUpdateInsertFlags(), tc.TreeConfig().PoolFactory())
				if err != nil {
			...
			for _, path := range tt.revertPaths {
				tc.OperationState().NonRevertiveInfo().Add(owner1, false, path)
			}
