...
func TestExplicitDeleteVisitor_Visit(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			root: func() *tree.RootEntry {
				...
				_, err = testhelper.LoadYgotStructIntoTreeRoot(ctx, testhelper.Config1(), root.Entry, owner1, owner1Prio, false, flagsNew)
				if err != nil {
					t.Error(err)
				}
				return root
		...
		},
		{
			...
			root: func() *tree.RootEntry {
				...
				_, err = testhelper.LoadYgotStructIntoTreeRoot(ctx, testhelper.Config1(), root.Entry, owner1, owner1Prio, false, flagsExisting)
				if err != nil {
					t.Error(err)
				}

				_, err = testhelper.LoadYgotStructIntoTreeRoot(ctx, testhelper.Config1(), root.Entry, consts.RunningIntentName, consts.RunningValuesPrio, false, flagsExisting)
				if err != nil {
					t.Error(err)
				}
		...
		},
		{
			...
			root: func() *tree.RootEntry {
				...
				_, err = testhelper.LoadYgotStructIntoTreeRoot(ctx, &sdcio_schema.Device{Interface: map[string]*sdcio_schema.SdcioModel_Interface{
					"ethernet-1/1": {
						Name:        ygot.String("ethernet-1/1"),
						Description: ygot.String("mydesc"),
					},
				}}, root.Entry, owner2, owner2Prio, false, flagsNew)
				if err != nil {
					t.Error(err)
				}
				return root
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			root := tt.root()
			root.GetTreeContext().GetOperationState().ExplicitDeletes().Add(owner2, tt.priority, tt.explicitDeletes)
