...
func Test_sharedEntryAttributes_BlameConfigParallel(t *testing.T) {
	owner1 := "owner1"
	owner2 := "owner2"
	ctx := context.TODO()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			bcv := NewBlameConfigVisitorParallel(tt.includeDefaults)

			// feed in the GOFUNCMAXPROCS to enable parallel processing

			start := time.Now()
			got, err := ParallelWalk(ctx, treeRoot.sharedEntryAttributes, bcv, runtime.NumCPU())
			if err != nil {
				t.Error("BlameConfig() error =", err)
				return
			}
