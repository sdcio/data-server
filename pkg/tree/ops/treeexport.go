...
func TreeExport(e api.Entry, owner string, priority int32, orphan bool) (*tree_persist.Intent, error) {
	...
	explicitDeletes := e.GetTreeContext().OperationState().ExplicitDeletes().GetByIntentName(owner).ToPathSlice()
	...
	if rootExportEntry != nil || len(explicitDeletes) > 0 {
		return &tree_persist.Intent{
			...
			Priority:        priority,
			NonRevertive:    e.GetTreeContext().OperationState().NonRevertiveInfo().IsGenerallyNonRevertive(owner),
