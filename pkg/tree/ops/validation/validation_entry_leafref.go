...
func breadthSearch(ctx context.Context, e api.Entry, sdcpbPath *sdcpb.Path) ([]api.Entry, error) {
	var err error
	...
	for _, elem := range lrefPath {
		...
		err := resolveLeafrefKeyPath(ctx, e, elem.Keys)
		if err != nil {
			return nil, err
		}
...
func navigateLeafRef(ctx context.Context, e api.Entry) ([]api.Entry, error) {
	...
	foundEntries, err := breadthSearch(ctx, e, lrefPath)
	if err != nil {
		return nil, err
	}
...
func resolveLeafrefKeyPath(ctx context.Context, e api.Entry, keys map[string]*types.LrefPathElemKeyValue) error {
	// resolve keys
	for k, v := range keys {
...
func validateLeafRefs(ctx context.Context, e api.Entry, resultChan chan<- *types.ValidationResultEntry, stats *types.ValidationStats) {
	if e.ShouldDelete() {
		return
	}
	...
	if err != nil || len(entry) == 0 {
		...
		pathStr := e.SdcpbPath().ToXPath(false)
	...
	if entry[0].ShouldDelete() {
		...
		// if required, issue error
		resultChan <- types.NewValidationResultEntry(lv.Owner(), fmt.Errorf("missing leaf reference: failed resolving leafref %s for %s to path %s LeafVariant %v", lref, e.SdcpbPath().ToXPath(false), e.SdcpbPath().ToXPath(false), lv), types.ValidationResultEntryTypeError)
		return
	}
