...
func (r *RootEntry) SetNonRevertiveIntent(intentName string, nonRevertive bool) {
	r.GetTreeContext().OperationState().NonRevertiveInfo().Add(intentName, nonRevertive)
}
...
func (r *RootEntry) FinishInsertionPhase(ctx context.Context) error {
	...
	for deletePathPrio := range r.GetTreeContext().OperationState().ExplicitDeletes().Items() {
		for path := range deletePathPrio.PathItems() {
			...
			err = edp.Run(ctx, entry, r.GetTreeContext().TreeConfig().PoolFactory())
			if err != nil {
				return err
			}
